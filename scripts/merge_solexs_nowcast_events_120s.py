...
MERGED_OUTPUT = ROOT / 'output' / 'solexs_nowcast_catalog_merged_120s.csv'
...
print('Merged SoLEXS nowcast catalog results (120s gap)')
print('===================================')
print(f'Total original events: {len(solexs):,}')
print(f'Total merged events: {len(merged):,}')
print(f'Matched real flares caught: {matched_flare_caught} / {len(matched):,}')
...
print(f'False alarm rate (merged, 120s): {merged[~merged["MATCHED"]].shape[0] / len(merged):.3f}')
